...
class StageScrollingPhysics extends ScrollPhysics {
  ...
  int currentStage;
  List<double> stages;
  StageScrollingPhysics({ScrollPhysics parent, this.stages, this.currentStage = 0})
      : super(parent: parent);

  @override
  StageScrollingPhysics applyTo(ScrollPhysics ancestor) {
    return StageScrollingPhysics(
        parent: buildParent(ancestor), stages: this.stages, currentStage: this.currentStage);
  }
  ...
  Simulation createBallisticSimulation(ScrollMetrics position, double velocity) {
    ...
    if (target != position.pixels)
      return ScrollSpringSimulation(
          spring, position.pixels, target, position.pixels > target ? -200 : 200,
          tolerance: tolerance);
    return null;
  }
